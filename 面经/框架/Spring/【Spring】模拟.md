...
AOP：面向切面的编程，将代码中重复的部分抽取出来，使用动态代理技术，在不修改源码的基础上对方法进行增强。如果目标对象实现了接口，默认采用**JDK动态代理**，也可以强制使用**CGLIB**，如果目标对象没有实现接口，采用**CGLIB**的方式。常用的场景包括**权限认证**、**自动缓存**、**错误处理**，**统一日志打印**，**调试**和**事务**等。
